...
def compile_sass
  ...
  compilation = system "sass --compass -C -t expand app/pollinate/stylesheets/app.sass > Resources/app.jss"
end

def compile_coffee
  puts "Compiling CoffeeScript".blue
  paths = `find app/pollinate -name '*.coffee'`.split("\n")
  compilation = (
  system "coffee -p --join --bare #{paths.join(' ')} > Resources/pollinate.js" and
